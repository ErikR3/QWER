...
- **`Coordinator.cs`** — in progress, not yet functional. Owns an `EntityManager` and a `ComponentManager` instance. `CreateEntity()` currently just calls `em.AddEntity()` and hardcodes a `return 0` — it doesn't yet return the id `AddEntity` actually allocated (note `AddEntity` is `void`, so the created id isn't observable by the caller yet) or touch `ComponentManager`. Intended as the facade that ties `EntityManager` + `ComponentManager` + `SystemManager` together behind one API, per the standard ECS pattern this project follows. `EntityManager.RemoveEntity` and `ComponentManager.EntityDestroyed` still need to be wired together here (e.g. a `DestroyEntity(entityId)` calling both) once entity destruction is handled.
